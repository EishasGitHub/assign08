...
var selected_slot_index: int = 0  # Currently selected inventory slot (0-9)
var held_item: InvItem = null     # Currently held item

signal slot_changed(index: int)   # Signal for UI updates

func _read():
	Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
	update_held_item()  # Initialize held item
...
func _unhandled_input(event):
	...
	# Handle number key presses for slot selection (1-9, 0)
	if event is InputEventKey and event.pressed:
		if event.keycode >= KEY_1 and event.keycode <= KEY_9:
			selected_slot_index = event.keycode - KEY_1  # Keys 1-9 = slots 0-8
			update_held_item()
			slot_changed.emit(selected_slot_index)
		elif event.keycode == KEY_0:
			selected_slot_index = 9  # Key 0 = slot 9
			update_held_item()
			slot_changed.emit(selected_slot_index)
...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("remove_block"):
		if $Camera3D/RayCast3D.is_colliding():
			# First check what we're colliding with. ONLY GridMap has this method:
	...
	if Input.is_action_just_pressed("place_block"):
		if $Camera3D/RayCast3D.is_colliding() and held_item != null:
			if $Camera3D/RayCast3D.get_collider().has_method("place_block"):
				...
				# Find which block index corresponds to this item
...
# Function to connect to GridMap signals
...
func _on_block_destroyed(item: InvItem, position: Vector3):
	collect(item)
	update_held_item()  # Update held item in case inventory changed

# Updates what item the player is currently holding
...
# Uses one of the currently held item
func use_held_item():
	if selected_slot_index < inv.slots.size():
		var slot = inv.slots[selected_slot_index]
		if slot.item != null and slot.amount > 0:
			...
			if slot.amount <= 0:
				...
				slot.amount = 0
				held_item = null
			inv.update.emit()  # Update UI

# Gets the block index for placing based on the item
...
#
			...
			## First check what we're colliding with. ONLY GridMap has this method:
				...
				#var collision_point = $Camera3D/RayCast3D.get_collision_point()
				#var collision_normal = $Camera3D/RayCast3D.get_collision_normal()
				#$Camera3D/RayCast3D.get_collider().place_block(collision_point + collision_normal, 6)
#
	#move_and_slide()
#
#
